...
class Trainer(object):
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch, batch_samples in enumerate(self.test_dataloader):
                ...
                torch.cuda.synchronize()
                t_end = time.time()
                print('cost time: {}'.format(t_end - t_begin))
                ...
                path_sr = os.path.join(save_path, image_name[:-4] + '_sr.png')
                path_gt = os.path.join(save_path, image_name[:-4] + '_gt.png')
                path_lr = os.path.join(save_path, image_name[:-4] + '_lr.png')
                out_img = output[0].flip(dims=(0,)).clamp(0., 1.)
                gt_img = HR[0].flip(dims=(0,))
                lr_img = LR[0].flip(dims=(0,))

                # torchvision.utils.save_image(torch.stack([out_img, gt_img]), path)
                torchvision.utils.save_image(out_img, path_sr)
                torchvision.utils.save_image(gt_img, path_gt)
                torchvision.utils.save_image(lr_img, path_lr)
    ...
    def inference(self, LR_pad, Ref, Ref_down):
        self.net.eval()
        with torch.no_grad():
            output = self.net(LR_pad, Ref, Ref_down)
        return output[0]
