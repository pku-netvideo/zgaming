...
def test(model, test_input_handle, configs, itr):
    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        if batch_id <= configs.num_save_samples:
            for frame in range(configs.batch_size):
                frame_id = test_frame_id[frame].split('_')
                path = os.path.join(res_path, frame_id[0], frame_id[1])
                frame_id = int(frame_id[1])
                os.makedirs(path)
                for i in range(configs.input_length, configs.total_length):
                    name = str(i * configs.index_stride + frame_id) + '_gt.png'
                    file_name = os.path.join(path, name)
                    img_gt = np.uint8(test_ims[frame, i, :, :, :] * 255)
                    img_gt[:, : ,[0,2]] = img_gt[: ,: ,[2,0]]
                    cv2.imwrite(file_name, img_gt)
                for i in range(configs.input_length - 1, configs.total_length - 1):
                    name = str((i+1)*configs.index_stride + frame_id) + '_pd.png'
                    file_name = os.path.join(path, name)
                    img_pd = img_gen[frame, i, :, :, :]
                    img_pd = np.maximum(img_pd, 0)
                    img_pd = np.minimum(img_pd, 1)
                    img_pd = np.uint8(img_pd * 255)
                    img_pd[:, : ,[0,2]] = img_pd[: ,: ,[2,0]]
